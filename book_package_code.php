...
$query = "SELECT price, p_name FROM packages WHERE p_id='$b_id'";
$result = mysqli_query($con, $query);

if ($row = mysqli_fetch_assoc($result)) {
    $package_name = $row['p_name'];
    $monthly_book = $row['price'];
    ...
    $q = "INSERT INTO bookings ( checkin_date, checkout_date, cost, type, name) 
          VALUES ('$checkin_date', '$checkout_date', '$total_cost', '$type', '$package_name')";
...
if (mysqli_query($con, $q)) {
    ...
    echo "<p style='font-size: 18px; font-family: Arial, sans-serif;'><strong>Package Name:</strong> $package_name</p>";
